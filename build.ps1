...
# Preserve import samples across PyInstaller COLLECT (it deletes dist\HHConverter).
$dist = Join-Path $PSScriptRoot "dist\HHConverter"
$importBackup = Join-Path $PSScriptRoot "dist\_import_backup"
if (Test-Path (Join-Path $dist "import")) {
    if (Test-Path $importBackup) { Remove-Item $importBackup -Recurse -Force }
    Copy-Item -Recurse (Join-Path $dist "import") $importBackup
}
...
if (Test-Path $importBackup) {
    $importDest = Join-Path $dist "import"
    New-Item -ItemType Directory -Force -Path $importDest | Out-Null
    Copy-Item -Force -Recurse (Join-Path $importBackup "*") $importDest
    Remove-Item $importBackup -Recurse -Force
}
