...
- **CoinPoker** tournaments and cash (multi-table name tracking; optional “Coin as PS” export; Dropbox: raw tournament hands → `CoinPoker/{year}/{month}/` day files, merged + deduped)
...
- Optional Dropbox backup (raw PP/GG/UP/888/Chico; Coin tournaments to `CoinPoker/{year}/{month}/` day files with dedupe)
...
| CoinPoker | `CoinPoker Hand #` | Tournaments + cash; H2N Coin / optional PS; Dropbox: raw tournament hands → `CoinPoker/{year}/{month}/CoinPoker_{year}_{month}_{day}_0.txt` (merged + deduped by hand id; cash excluded) |
